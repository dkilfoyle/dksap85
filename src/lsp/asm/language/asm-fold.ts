...
import { FoldingRangeProvider, LangiumServices } from "langium/lsp";
...
export class AsmFoldProvider implements FoldingRangeProvider {
  ...
  getFoldingRanges(document: LangiumDocument, _params: FoldingRangeParams): FoldingRange[] {
      ...
      do {
        ...
        if (!result.done) {
          ...
          if (isLabel(node) && node.$cstNode) {
            let end = node.$cstNode.range.start.line;
