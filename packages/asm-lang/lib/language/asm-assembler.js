...
import { isProgram, isLabel, isDirective, isAddrArgument, isImm8, isReg16, isReg8, isImm16, isInstr } from "./generated/ast.js";
import { instructionInfo, opcodes } from "./opcodes.js";
const getInstructionInfo = (instr) => {
    ...
    if (instructionInfo[instrKey] == undefined)
        throw Error(`No instruction info for ${instrKey}`);
    return instructionInfo[instrKey];
};
export const assember = (root) => {
    if (!isProgram(root))
        throw Error("Assembler expects Program node as root");
    const identifierMap = {};
    const lineOpcodeMap = {};
    const lineAddressMap = {};
    let addr = 0;
    ...
    do {
        ...
        if (!result1.done) {
            ...
            if (lineNum == undefined)
                throw Error("Assembler linenum undefined");
            if (isInstr(node)) {
                    ...
                    case "ORG":
                        {
                            ...
                            if (a == undefined)
                                throw Error("ORG expects number");
                            addr = a;
                        }
                        break;
                    case "EQU":
                        if (!node.lhs?.identifier)
                            throw Error("EQU missing lhs.identifier");
                    ...
                    case "DW":
                        node.args.forEach((arg) => {
                            if (arg.number != undefined) {
                                ...
                                if (x == undefined)
                                    throw Error("Identifer in DW arg not found");
                                bytes[addr++] = x & 0xff;
                                bytes[addr++] = (x >> 8) & 0xff;
                            }
                            else
